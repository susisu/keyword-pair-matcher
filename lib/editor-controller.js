...
const generateMatches = pairs => {
  ...
  for (const [start, end] of pairs) {
    // ignore if conflict
    if (endMatches.has(start) || startMatches.has(end)) {
      continue;
    }
...
export class EditorController {
  ...
  update() {
    ...
    if (this.isCommentedOrString(this.editor.getCursorBufferPosition())) {
      return;
    }
    ...
    if (this.startMatches.has(findResult.keyword)) {
      start = findResult;
      end = this.findKeywordForward(findResult);
    }
    else if (this.endMatches.has(findResult.keyword)) {
      start = this.findKeywordBackward(findResult);
      end = findResult;
    }
    if (!start || !end) {
      return;
  ...
  findKeywordForward({ keyword, range }) {
  ...
  findKeywordBackward({ keyword, range }) {
    ...
    this.editor.backwardsScanInBufferRange(this.keywordRegex, scanRange, scanResult => {
      ...
      if (this.endMatches.has(matchText)) {
        ...
        if (this.startMatches.get(matchText).has(stack[stack.length - 1])) {
          stack.pop();
        }
      }
      if (stack.length === 0) {
