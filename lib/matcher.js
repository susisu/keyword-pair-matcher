...
export class Matcher {
  ...
  updateMatches(pairs) {
    ...
    if (pairs.length > 0 && parsedPairs.length === 0) {
      atom.notifications.addWarning("keyword-pair-matcher", {
        dismissable: true,
        detail     : "Keyword pairs separated by a slash (/) are deprecated. Use dots (..) instead.",
      });
      parsedPairs = oldParsePairs(pairs);
    }
    const { startMatches, endMatches, keywordRegex } = generateMatches(parsedPairs);
